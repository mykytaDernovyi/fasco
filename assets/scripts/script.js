...
const Cart = (() => {
  ...
  const getFreeShippingRemaining = () =>
    Math.max(0, FREE_SHIPPING - getTotalPrice());
  const getFreeShippingProgress = () =>
    Math.min(100, (getTotalPrice() / FREE_SHIPPING) * 100);
...
const Wishlist = (() => {
  ...
  const save = () => {
    ...
    try {
      window.FascoApp?.notifyWishlistChanged?.();
    } catch {
      /* ignore */
    }
  };
...
function refreshWishlistButtons() {
  document.querySelectorAll("[data-wishlist-id]").forEach((btn) => {
    ...
    if (icon)
      icon.className = (isWish ? "fa-solid" : "fa-regular") + " fa-heart";
  });
}
...
window.addEventListener("fasco:remote-sync", () => {
  try {
    ...
    console.warn("[Fasco] remote-sync error", e);
  }
});
...
const CartDrawer = (() => {
  ...
  function updateShippingBar() {
    ...
    if (costEl) {
      ...
      costEl
        .closest(".cart-total-row")
        ?.classList.toggle("cart-total-row--muted", remaining > 0);
    }
  }
  ...
  function render() {
    ...
    list
      .querySelectorAll(".ci-minus")
      .forEach((btn) =>
        btn.addEventListener("click", () =>
          Cart.updateQty(+btn.dataset.idx, -1),
        ),
      );
    list
      .querySelectorAll(".ci-plus")
      .forEach((btn) =>
        btn.addEventListener("click", () =>
          Cart.updateQty(+btn.dataset.idx, 1),
        ),
    ...
    list
      ...
      .forEach((btn) =>
        btn.addEventListener("click", () => Cart.remove(+btn.dataset.idx)),
      );
  }
  ...
  function init() {
    document.addEventListener("DOMContentLoaded", () => {
      ...
      $("stickyCartLink")?.addEventListener("click", (e) => {
        e.preventDefault();
        open();
      });
      ...
      if (addBtn) {
        addBtn.addEventListener("click", () => {
          ...
          const size =
            sizeEl?.dataset?.size?.trim() || sizeEl?.innerText?.trim() || "M";
  ...
  function render() {
    ...
    items.forEach((item) => {
      const oldPriceHTML = item.oldPrice
        ? `<span class="wishlist-item-old-price">$${item.oldPrice.toFixed(2)}</span>`
        : "";
      const div = document.createElement("div");
...
const SearchOverlay = (() => {
  ...
  async function loadProducts() {
    ...
    try {
      ...
      console.warn("Search: could not load products");
    }
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.addEventListener("resize", () => {
    if (window.innerWidth >= 1024 && panel.classList.contains("open"))
      closeMenu();
  });
});
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (urlCollection) {
    const link = document.querySelector(
      `#collectionList a[data-collection="${urlCollection}"]`,
    );
    if (link) {
      activeFilters.collection = urlCollection;
      document
        .querySelectorAll("#collectionList a")
        .forEach((a) => a.classList.remove("active-collection"));
  ...
  async function loadProducts() {
    try {
      const [productsRes, stocks] = await Promise.all([
        ...
        window.FascoApp?.getAllStock?.().catch(() => ({})) ??
          Promise.resolve({}),
      ]);
      ...
      productGrid.innerHTML =
        '<p style="grid-column:1/-1;text-align:center;padding:40px;color:#aaa;">Помилка завантаження.</p>';
    }
  }
  ...
  function availableSizesOf(p) {
    ...
    colors.forEach((byColor) =>
      Object.keys(byColor || {}).forEach((s) => allSizes.add(s)),
    );
    const set = new Set();
    allSizes.forEach((size) => {
      ...
      if (
        relevant.length &&
        relevant.every((byColor) => Number(byColor[size]) > 0)
      )
        set.add(size);
    });
    return set;
  ...
  function applyFilters(products) {
    return products.filter((p) => {
      if (
        ...
        !(p.collections || []).includes(activeFilters.collection)
      )
        return false;
      ...
      if (activeFilters.colors.length) {
        const ok = activeFilters.colors.some((c) =>
          (p.swatches || []).some((s) => s.toLowerCase() === c.toLowerCase()),
        );
        if (!ok) return false;
      }
  ...
  function renderPage() {
    ...
    displayProducts(
      products.slice(start, start + itemsPerPage),
      products.length === 0,
    );
  ...
  function displayProducts(products, isEmpty) {
    ...
    products.forEach((product) => {
      ...
      const saleBadge = product.oldPrice
        ? `<span class="sale-badge-small">Sale</span>`
        : "";
  ...
  function setupPagination(total) {
    ...
    if (currentPage > 1)
      paginationContainer.appendChild(addLink("«", currentPage - 1));
  ...
  document
    .querySelectorAll(".filter-group a[data-price-min]")
    .forEach((link) => {
      link.addEventListener("click", (e) => {
        e.preventDefault();
        const min = parseFloat(link.dataset.priceMin),
          max = parseFloat(link.dataset.priceMax);
        if (activeFilters.priceMin === min && activeFilters.priceMax === max) {
          ...
          document
            .querySelectorAll(".filter-group li.active-price")
            .forEach((li) => li.classList.remove("active-price"));
  ...
  document
    .querySelectorAll(".filter-group.collapsible .filter-group-header")
    .forEach((header) => {
      header.addEventListener("click", () =>
        header.closest(".filter-group.collapsible")?.classList.toggle("closed"),
      );
    });

  document
    .querySelectorAll("#collectionList a[data-collection]")
    .forEach((link) => {
      link.addEventListener("click", (e) => {
        e.preventDefault();
  ...
  function renderActiveFilterTags() {
        ...
        makeTag(
          ...
          () => {
            ...
            document
              ...
              .forEach((a) => a.classList.remove("active-collection"));
            document
              .querySelector('#collectionList a[data-collection="all"]')
              ?.classList.add("active-collection");
            const url = new URL(location.href);
    ...
    activeFilters.sizes.forEach((size) => {
      bar.appendChild(
        makeTag(`${size} <i class="fa-solid fa-xmark"></i>`, () => {
          activeFilters.sizes = activeFilters.sizes.filter((s) => s !== size);
          document
            .querySelector(`.size-box[data-size="${size}"]`)
      ...
      bar.appendChild(
        makeTag(
          ...
          () => {
            ...
            document
              .querySelector(`.color-dot[data-color="${color}"]`)
              ?.classList.remove("active");
            currentPage = 1;
            renderPage();
          },
    ...
    if (searchQuery) {
      bar.appendChild(
        makeTag(`"${searchQuery}" <i class="fa-solid fa-xmark"></i>`, () => {
          searchQuery = "";
          const input = $("searchInput");
          if (input) input.value = "";
          currentPage = 1;
  ...
  function updateFilterResetBtn() {
    ...
    if (!btn) {
      ...
      btn.addEventListener("click", () => {
        ...
        searchQuery = "";
        document
          .querySelectorAll(".size-box.active")
          .forEach((b) => b.classList.remove("active"));
        document
          .querySelectorAll(".color-dot.active")
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (!productId) {
    productTitleEl.innerText = "Оберіть товар у магазині";
    hideLoader();
    return;
  }
  ...
  try {
    const [res, initialStock] = await Promise.all([
      fetch("./assets/data/products.json"),
      window.FascoApp?.getStock?.(productId).catch(() => null) ??
        Promise.resolve(null),
    ]);
    const product = (await res.json()).find((p) => p.id === productId);
    if (!product) {
      productTitleEl.innerText = "Товар не знайдено";
      hideLoader();
      return;
    }
    ...
    function renderGalleryLayout() {
      ...
      if (isMobileLayout) {
        ...
        if (!staticImg) {
          ...
          mainImageContainer.insertBefore(
            staticImg,
            mainImageContainer.firstChild,
          );
        }
    ...
    function loadGallery(images) {
      ...
      if (!thumbsEl) return;
      thumbsEl.innerHTML = currentImages
        .map(
          (src, i) =>
    ...
    if (mainImgContainer) {
      let touchStartX = 0;
      mainImgContainer.addEventListener(
        "touchstart",
        (e) => {
    ...
    function renderStockText(size) {
      ...
      if (left <= 0) {
        stockText.textContent = "Розпродано";
        stockText.className = "product-stock-text is-out";
      } else if (left <= 3) {
        stockText.textContent = `Залишилось ${left} шт`;
        stockText.className = "product-stock-text is-low";
      } else {
        stockText.textContent = `В наявності: ${left} шт`;
        stockText.className = "product-stock-text";
      }
    }
    ...
    function renderSizes() {
      ...
      const stillAvailable =
        order.includes(activeSizeKey) && Number(sizes[activeSizeKey]) > 0;
      const next = stillAvailable ? activeSizeKey : pickInitialSize();
      sizesWrap.innerHTML = order
        .map((s) => {
    ...
    if (colorContainer && product.swatches?.length) {
      colorContainer.innerHTML = product.swatches
        .map(
          (color, i) =>
  ...
  if (tH && tM && tS) {
    const tick = () => {
      ...
      tM.innerText = String(Math.floor((totalSeconds % 3600) / 60)).padStart(
        2,
        "0",
      );
...
// === REVIEWS (PDP) ===
// Підвантажуємо відгуки з Firestore та рендеримо в табі + в шапці товару.
// HTML не чіпаємо — список/кнопку/модалку додаємо в DOM динамічно.
...
const starsHTML = (rating) => {
  ...
  let html = "";
  for (let i = 1; i <= 5; i++) {
    html += `<i class="${i <= r ? "fa-solid" : "fa-regular"} fa-star"></i>`;
  }
  return html;
};
...
function initReviews(productId) {
  ...
  // Додаємо блоки після summary: кнопку + список.
  const actions = document.createElement("div");
  actions.className = "reviews-actions";
  actions.innerHTML = `<button type="button" class="btn-leave-review">Залишити відгук</button>`;

  const listEl = document.createElement("div");
  listEl.className = "reviews-list";

  summaryEl.after(actions, listEl);

  // ID мого відгуку (якщо є) — щоб блокувати повторне додавання.
  let myReviewId = null;

  // onSnapshot — жива підписка: будь-яка зміна в Firestore
  // одразу прилітає сюди і ми перемальовуємо весь блок.
  window.FascoApp?.watchReviews?.(productId, (reviews) => {
    const count = reviews.length;
    const avg = count ? reviews.reduce((s, r) => s + r.rating, 0) / count : 0;
    const meUid = window.FascoApp?.user?.uid;
    myReviewId = reviews.find((r) => r.userId === meUid)?.id || null;

    // 1. Шапка товару: зірочки + "(4.2) 12 відгуків"
    const headStars = document.querySelector(".rating-reviews .stars-row");
    const headCount = document.querySelector(".rating-reviews .reviews-count");
    if (headStars) headStars.innerHTML = starsHTML(avg);
    if (headCount)
      headCount.innerText = count
        ? `(${avg.toFixed(1)}) ${count} відгуків`
        : "— немає відгуків";

    // 2. Назва таба "Відгуки (N)"
    const tabBtn = document.querySelector('[data-tab="reviews"]');
    if (tabBtn) tabBtn.innerText = `Відгуки (${count})`;

    // 3. Summary-блок усередині таба
    summaryEl.innerHTML = `
            <div class="rating-big">${count ? avg.toFixed(1) : "—"}</div>
            <div class="rating-meta">
                <div class="rating-stars-big">${starsHTML(avg)}</div>
                <p class="regular-text">На основі ${count} відгуків</p>
            </div>
        `;
    ...
    if (!count) {
      listEl.innerHTML =
        '<p class="pdp-reviews-empty">Поки немає відгуків. Будьте перші.</p>';
    } else {
      listEl.innerHTML = reviews
        .map(
          (r) => `
                <div class="pdp-review-card" data-id="${r.id}">
                    <div class="pdp-review-head">
                        <div class="pdp-review-avatar">${escapeHtml((r.userName || "?").trim().charAt(0).toUpperCase())}</div>
                        <div class="pdp-review-meta">
                            <span class="pdp-review-name">${escapeHtml(r.userName)}</span>
                            <span class="pdp-review-date">${fmtReviewDate(r.createdAt)}</span>
                        </div>
                        <div class="pdp-review-stars">${starsHTML(r.rating)}</div>
                    </div>
                    <p class="pdp-review-text">${escapeHtml(r.text)}</p>
                    ${r.userId === meUid ? '<button class="pdp-review-delete" type="button">Видалити</button>' : ""}
                </div>
            `,
        )
        .join("");
  ...
  // Делегований клік: видалення свого відгуку.
  listEl.addEventListener("click", async (e) => {
    const btn = e.target.closest(".pdp-review-delete");
    if (!btn) return;
    const id = btn.closest(".pdp-review-card")?.dataset.id;
    if (!id || !confirm("Видалити ваш відгук?")) return;
    try {
      await window.FascoApp?.deleteReview?.(productId, id);
    } catch (err) {
      alert(err?.message || "Не вдалося видалити.");
    }
  });

  // Клік "Залишити відгук".
  actions.querySelector(".btn-leave-review").addEventListener("click", () => {
    let mode = "form";
    if (!window.FascoApp?.isAuthed?.()) mode = "signin";
    else if (myReviewId) mode = "duplicate";
    openReviewModal(productId, mode);
  });
}

function openReviewModal(productId, mode = "form") {
  if (!_reviewModal) _reviewModal = buildReviewModal();
  const { modal, title, info, form, stars, textEl, errorEl } = _reviewModal;

  // Скидаємо обидва екрани (форма + інфо-повідомлення).
  form.hidden = true;
  info.hidden = true;
  info.innerHTML = "";
  errorEl.textContent = "";
  ...
  if (mode === "signin") {
    title.textContent = "Потрібен вхід";
    info.innerHTML = `
            <p class="review-info-text">Щоб залишити відгук, увійдіть у свій акаунт.</p>
            <div class="review-info-actions">
                <a class="btn btn-modal" href="signin.html">Увійти</a>
                <a class="btn btn-modal btn-modal-secondary" href="signup.html">Створити акаунт</a>
            </div>
        `;
    info.hidden = false;
  } else if (mode === "duplicate") {
    title.textContent = "Ви вже залишили відгук";
    info.innerHTML = `
            <p class="review-info-text">Ваш відгук на цей товар уже опубліковано.<br>Видалити його можна в списку нижче.</p>
            <div class="review-info-actions">
                <button type="button" class="btn btn-modal review-info-close">Зрозуміло</button>
            </div>
        `;
    info.hidden = false;
    info.querySelector(".review-info-close").onclick = closeReviewModal;
  } else {
    ...
    let chosen = 0;
    const paint = (n, cls) =>
      stars.forEach((s, i) => s.classList.toggle(cls, i < n));
    paint(0, "chosen");
    paint(0, "hover");
    textEl.value = "";

    stars.forEach((s, i) => {
      s.onmouseenter = () => paint(i + 1, "hover");
      s.onmouseleave = () => paint(0, "hover");
      s.onclick = () => {
        chosen = i + 1;
        paint(chosen, "chosen");
      };
    ...
    form.onsubmit = async (e) => {
      ...
      errorEl.textContent = "";
      if (!chosen) {
        errorEl.textContent = "Виберіть оцінку від 1 до 5 зірок.";
        return;
      }
      try {
        await window.FascoApp.addReview(productId, {
          rating: chosen,
          text: textEl.value,
        });
        closeReviewModal();
      } catch (err) {
        errorEl.textContent = err?.message || "Не вдалося надіслати відгук.";
      }
    };
  }
...
function closeReviewModal() {
  if (!_reviewModal) return;
  const modal = _reviewModal.modal;
  modal.classList.remove("open");
  modal.classList.add("close");
  setTimeout(() => modal.classList.remove("close"), 400);
  document.body.style.overflow = "";
...
function buildReviewModal() {
  ...
  modal.innerHTML = `
        <div class="modal-content">
            <span class="close-modal">&times;</span>
            <h2 class="font-title review-modal-title">Залишити відгук</h2>
            <div class="product-review-info" hidden></div>
            <form class="modal-form review-form" hidden>
                <div class="star-input" aria-label="Оцінка">
                    ${[1, 2, 3, 4, 5].map((n) => `<i class="fa-regular fa-star" data-n="${n}"></i>`).join("")}
                </div>
                <textarea placeholder="Що сподобалось, а що ні?" maxlength="1000" rows="4" required></textarea>
                <p class="review-form-error" aria-live="polite"></p>
                <button type="submit" class="btn btn-modal">Опублікувати</button>
            </form>
        </div>
    `;
  document.body.appendChild(modal);

  modal.querySelector(".close-modal").onclick = closeReviewModal;
  ...
  return {
    modal,
    title: modal.querySelector(".review-modal-title"),
    info: modal.querySelector(".product-review-info"),
    form: modal.querySelector(".review-form"),
    stars: [...modal.querySelectorAll(".star-input i")],
    textEl: modal.querySelector("textarea"),
    errorEl: modal.querySelector(".review-form-error"),
  };
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (giftWrapCheck) {
    try {
      giftWrapCheck.checked = localStorage.getItem("fasco_giftwrap") === "1";
    } catch {
      /* ignore */
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const syncSaleBox = () => {
    ...
    if (saleName && first.dataset.name)
      saleName.textContent = first.dataset.name;
  ...
  const syncDots = () => {
    ...
    dotEls.forEach((d, i) =>
      d.classList.toggle("progress-highlight", i === idx),
    );
  };
...
(function () {
  ...
  const getGiftWrap = () => {
    try {
      return localStorage.getItem("fasco_giftwrap") === "1";
    } catch {
      return false;
    }
  };
  ...
  function renderTotals() {
    ...
    if (discountRow) {
      ...
      if (promoPercent > 0 && $("ckDiscount"))
        $("ckDiscount").textContent = "-" + fmt(t.discount);
    }
    const giftRow = $("ckGiftWrapRow");
    if (giftRow) {
      giftRow.style.display = t.giftWrap > 0 ? "" : "none";
      if (t.giftWrap > 0 && $("ckGiftWrap"))
        $("ckGiftWrap").textContent = fmt(t.giftWrap);
    }
    if ($("ckShipping"))
      $("ckShipping").textContent =
        t.shipping === 0 ? "Безкоштовно" : fmt(t.shipping);
    ...
    form.addEventListener("submit", async (e) => {
      ...
      for (const id of required) {
        ...
        if (el.type === "email" && !/^\S+@\S+\.\S+$/.test(el.value))
          invalid = true;
        if (id === "ck-cardnum" && el.value.replace(/\s/g, "").length < 16)
          invalid = true;
      ...
      if (btn) {
        btn.disabled = true;
        btn.innerHTML =
          '<i class="fa-solid fa-spinner fa-spin"></i><span>Зберігаємо…</span>';
      }
      ...
      const order = {
        ...
        paymentMethod:
          form.querySelector('input[name="payment-method"]:checked')?.value ||
          "card",
        marketingOptIn: !!document.querySelector(
          '.checkout-block input[type="checkbox"]:not([id])',
        )?.checked,
      };
      ...
      try {
        ...
        alert(
          err?.message || "Не вдалося оформити замовлення: товар закінчився.",
        );
        return;
      }
      ...
      if ($("ck-save-info")?.checked && window.FascoApp?.updateUserProfile) {
        try {
          ...
          console.warn("[Fasco] save-info failed", e);
        }
      }

      let orderId = null;
      try {
        orderId = await window.FascoApp?.saveOrder?.(order);
      } catch (err) {
        console.warn("[Fasco] order save failed", err);
      }
      ...
      if (btn) {
        btn.innerHTML =
          '<i class="fa-solid fa-circle-check"></i><span>Замовлення оформлено</span>';
  ...
  function fillFromProfile(user, profile) {
    ...
    set(
      "ck-lastname",
      p.lastName || user.displayName?.split(" ").slice(1).join(" ") || "",
    );
    ...
    document.querySelectorAll(".checkout-account-link").forEach((el) => {
...
(function () {
  ...
  function renderHeaderAvatar(user, profile) {
    ...
    if (!user) {
      wrapper.innerHTML =
        '<a href="signin.html" id="userToolBtn" aria-label="Акаунт"><i class="fa-regular fa-user"></i></a>';
      return;
    }
  ...
  function ensureModal() {
    ...
    m.querySelector(".profile-modal-backdrop").addEventListener(
      "click",
      closeProfileModal,
    );
    m.querySelector(".profile-modal-close").addEventListener(
      "click",
      closeProfileModal,
    );
    m.querySelector("#profileModalLogout").addEventListener(
      "click",
      async () => {
    ...
    m.querySelector("#profileModalForm").addEventListener(
      "submit",
      async (e) => {
        e.preventDefault();
    ...
    m.querySelectorAll(".profile-tab").forEach((tab) => {
      tab.addEventListener("click", () => {
        ...
        m.querySelectorAll(".profile-tab").forEach((t) =>
          t.classList.toggle("is-active", t === tab),
        );
        m.querySelectorAll(".profile-tab-pane").forEach((p) =>
  ...
  const paymentLabel = (p) =>
    ({ card: "Картка", apple: "Apple Pay", cod: "Готівка при отриманні" })[p] ||
    p ||
    "—";
  const statusLabel = (s) =>
    ({
  ...
  async function loadOrders() {
    ...
    try {
      ...
      if (!orders.length) {
        pane.innerHTML =
          '<p class="profile-orders-empty">У вас ще немає замовлень.</p>';
        return;
      }
      pane.innerHTML = orders
        .map((o) => {
          const t = o.totals || {};
          const itemsHtml = (o.items || [])
            ...
            .join("");
          const promo = t.promoCode
            ? ` <span class="order-promo">(${t.promoCode} −${t.promoPercent}%)</span>`
            : "";
          return `
      ...
      pane.innerHTML =
        '<p class="profile-orders-empty">Не вдалося завантажити замовлення.</p>';
    }
  }
  ...
  function openProfileModal() {
    ...
    const ln =
      ...
      user.displayName?.split(" ").slice(1).join(" ") ||
      "";
    m.querySelector("#profileModalTitle").textContent =
      `${fn} ${ln}`.trim() || "Профіль";
    ...
    m.querySelectorAll(".profile-tab").forEach((t) =>
      t.classList.toggle("is-active", t.dataset.tab === "profile"),
    );
    m.querySelectorAll(".profile-tab-pane").forEach((p) =>
      p.classList.toggle("is-active", p.dataset.pane === "profile"),
    );
    ...
    window.FascoApp.onAuthChanged((user, profile) => {
      ...
      if (user && sessionStorage.getItem("fasco_open_orders") === "1") {
        ...
        setTimeout(() => {
          openProfileModal();
          $(PROFILE_MODAL_ID)
            ?.querySelector('.profile-tab[data-tab="orders"]')
            ?.click();
        }, 200);
      }
    });
...
(function initNavDropdown() {
  function bind() {
    document
      .querySelectorAll(".desktop-nav-links .has-dropdown")
      .forEach((item) => {
        ...
        trigger.addEventListener("click", (e) => {
          ...
          document
            .querySelectorAll(".desktop-nav-links .has-dropdown.open")
            .forEach((other) => {
...
(function initSubscribeToast() {
  function showToast(kind, title, message) {
    ...
    if (icon)
      icon.className =
        "subscribe-toast-icon fa-solid " +
        (isError ? "fa-circle-xmark" : "fa-circle-check");
    ...
    function handleSubmit() {
      ...
      if (!isValidEmail(email)) {
        ...
        showToast(
          ...
          "Невірний email",
          email
            ? "Перевірте формат — має бути на кшталт name@example.com."
            : "Введіть, будь ласка, вашу email-адресу.",
        );
        return;
      }
      ...
      showToast(
        "success",
        "Дякуємо за підписку!",
        "Ми надішлемо вам новини про колекції та знижки на вказану пошту.",
      );
      input.value = "";
      input.blur();
    }
